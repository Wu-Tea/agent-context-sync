...
- 超过约 80 行时提示可以压缩。
- 超过约 120 行时强烈建议压缩。
- 对“已经完成很久且不再影响下一次接手”的内容，优先下沉到 `session-log.md`、`decisions/` 或 archive，而不是继续堆在 handoff 里。

## 10. `session-log.md` 需求

`session-log.md` 用于记录重要节点，并继续作为主读取入口。每条记录应包含:
...
`session-log.md` 需要兼容 index + content 的渐进式结构:

- 默认可以按时间追加重要 session 摘要。
- 当主日志过长时，允许压缩为:
  - archive note
  - milestone summary
  - recent active checkpoints
- 详细历史可以下沉到可选目录 `.agent-context/archive/`。
- 即使发生压缩，主入口文件仍然叫 `session-log.md`，不要求引入新的强制入口文件。
- archive 是可选增强，不应成为每次启动都必须读取的文件。
- 写入前检查当前行数:
  - 超过约 100 行时提示可以压缩
  - 超过约 160 行时强烈建议压缩
...
- `session-log.md` 是否已经过长到不适合作为启动主入口。
- 如果已经归档，`session-log.md` 是否仍然保留了对 archive 的清晰链接。
...
- handoff / session-log 的压缩阈值是否需要允许用户项目级调整。
